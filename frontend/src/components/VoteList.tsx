...
export default function VoteList({ votes }: Props) {
  if (votes.length === 0) {
    return (
      <section className="bg-white rounded-xl border border-slate-100 shadow-sm p-6 text-slate-500 text-center">
  ...
  return (
    <section className="bg-white rounded-xl border border-slate-100 shadow-sm p-6">
      <h2 className="text-xl font-bold text-slate-900">Avis de la communauté</h2>
      ...
      <div className="mt-5 space-y-4">
        {votes.map((vote) => (
          ...
          >
            ...
            {vote.comment && (
              <p className="mt-3 text-slate-600">{vote.comment}</p>
            )}
