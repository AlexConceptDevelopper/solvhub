...
export default function SolutionStatsCard({ stats }: Props) {
  const total = stats.successCount + stats.partialCount + stats.failureCount;

  const score =
    total === 0
      ? 0
      : ((stats.successCount + stats.partialCount * 0.5) / total) * 100;
    ...
    <section className="bg-white rounded-xl border border-slate-100 shadow-sm p-6">
      ...
      <div className="mt-5 grid grid-cols-3 gap-4 text-center">
        <div>
          ...
          <p className="text-sm text-slate-500">Réussies</p>
        </div>

        <div>
          <p className="text-2xl font-bold text-amber-600">{stats.partialCount}</p>
          <p className="text-sm text-slate-500">Partielles</p>
        </div>
      ...
      <div className="mt-6 text-center text-slate-600">
        Score communauté :
        <span className="ml-2 font-bold text-blue-600">{score.toFixed(0)}%</span>
      </div>
    </section>
  );
}