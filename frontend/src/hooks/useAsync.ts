...
import useAsync from "../../hooks/useAsync"; // Ajuste le chemin selon l'emplacement de ton hook
...
export default function UsersList({ users, setUsers, search }: UsersListProps) {
  ...
  // --- Utilisation de useAsync pour les actions asynchrones ---
  const { execute: executeAction, loading: actionLoading } = useAsync<any>();
  ...
  const confirmDelete = async () => {
    ...
    const result = await executeAction(() => deleteUser(userToDelete));
    if (result !== null) {
  ...
  // Enregistrer les modifications via la modale avec useAsync
  const saveEditing = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!editingUser) return;

    const updatedUser = await executeAction(() =>
  ...
  return (
    <div className="space-y-6 relative">
                  ...
                  <td className="p-4 text-right">
                    <div className="flex justify-end gap-2">
                      <button
                        ...
                        className="px-3 py-2 font-bold text-white bg-blue-600 hover:bg-blue-700 rounded shadow-sm cursor-pointer text-xs transition disabled:opacity-50"
                      >
                        Modifier
                      </button>
                      <button
                        ...
                        disabled={actionLoading}
                        className="px-3 py-2 font-bold text-white bg-red-600 hover:bg-red-700 rounded shadow-sm cursor-pointer text-xs transition disabled:opacity-50"
                      >
                        Supprimer
                      </button>
                    </div>
      ...
      <ConfirmModal
        ...
        confirmText={actionLoading ? "Suppression..." : "Oui, supprimer"}
        variant="danger"
